...
fins <- c("Results/Genes/out_sup_CLTC.fasta","Results/Genes/out_sup_CLTCL1.fasta")
...
fin <- fins[2]
	...
	ind <- which(res[seq(2,length(res),2)] %in% as.character(df[df$Freq>20,1]))
...
ind <- unique(c(ind, (length(res)/2)-1, (length(res)/2)))
	...
	plot(net, size=ss, scale.ratio=50, pie=ind.hap, labels=T, legend=F, show.mutation=0, th=0, fast=F, main="B")
	mtext("CLTCL1",3,0)
        legend("topleft", colnames(ind.hap), col=rainbow(ncol(ind.hap)), pch=19, ncol=2)
...
fin <- "Results/Chimp/CLTC.pop.fasta_0_0.bin.haps.fasta"
res <- readLines(fin)
heads <- res[seq(1,length(res),2)]
haps <- res[seq(2,length(res),2)]
counts <- read.csv("Results/Chimp/CLTC.pop.fasta_0_0.bin.traits.txt")
cat("", file="tmp.chimp.fasta")
for (i in 1:nrow(counts)) {
        for (j in 2:ncol(counts)) {
                volte <- counts[i,j]
...
ss <- (attr(net, "freq"))
#ss[ss<3] <- 6
plot(net, size=ss, scale.ratio=300, pie=ind.hap, labels=F, legend=F, show.mutation=0, th=0)
mtext("CLTC",3,0)
legend("bottomleft", colnames(ind.hap), col=rainbow(ncol(ind.hap)), pch=19, ncol=2)
