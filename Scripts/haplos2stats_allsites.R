...
fout <- paste("Results/Genes_allsites/",gene,".stats.txt",sep="",collapse="")

# read haplos for all populations

f_freqs <- paste("Results/Genes_allsites/out_pop_",gene,".fasta_0_0.traits.txt",sep="",collapse="")
f_seqs <- paste("Results/Genes_allsites/out_pop_", gene,".fasta_0_0.haps.fasta",sep="",collapse="")

haps <- trimws(readLines(f_seqs)); haps=haps[seq(2,length(haps),2)]
freqs <- read.csv(f_freqs, head=T)
pops <- colnames(freqs)[-1]

# here I assume there are no altai, denisovan, chimp!
