...
# check whether ape snps in vcf are nonsyno or not
# also extract 5' promoter region
# extract reference gene sequence hg18 vcf file

# retrieve table with coding exons (from cds start to end, and strand)
refgene <- read.table("Data/refGene_hg18.txt")
colnames(refgene) <- strsplit(readLines("Data/refGene_hg18.txt")[1], split="\t")[[1]]
...
for (i in 1:length(bases)) {
	cmd=paste(paste("~/Software/samtools-1.9/samtools faidx ~/Data/hg18/chr", chrom, ".fa.gz", sep="", collapse=""), paste(as.character(refgene$chrom),":",paste(rep(bases[i],2),sep="",collapse="-"),sep="",collapse=""))
...
for (i in 1:length(proms)) {
	        cmd=paste(paste("~/Software/samtools-1.9/samtools faidx ~/Data/hg18/chr", chrom, ".fa.gz", sep="", collapse=""), paste(as.character(refgene$chrom),":",paste(rep(proms[i],2),sep="",collapse="-"),sep="",collapse=""))
...
# n_line_header
n_line_header=6
fin=paste("~/Data/CLTCL1/Apes/", genename, ".phased.vcf", sep="", collapse="")
...
for (i in 1:nsites) {
	...
	if (!is.na(ind[i])) {
		...
		if (strand=="-") {
			if (!(as.character(vcf[i,4])==reverse(substring(mine,ind[i],ind[i])))) stop("references do not match!");
		} else {
			if (!(as.character(vcf[i,4])==(substring(mine,ind[i],ind[i])))) stop("references do not match!");
		}
...
write.table(anno, file=paste("Results/Apes/", genename,".anno.txt", sep="", collapse=""), col.names=T, row.names=F, quote=F, sep="\t")
cat(genomic, file=paste("Results/Apes/", genename,".promoter.txt", sep="", collapse=""), sep="\n")

# write fasta but for the whole protein

fout=paste("Results/Apes/", genename,".fasta",sep="",collapse="")
cat("", file=fout)
for (i in 1:nind) {
...
fout=paste("Results/Apes/", genename,".promoter.fasta",sep="",collapse="")
cat("", file=fout)
for (i in 1:nind) {
        cat(paste(">",names[i],sep="",collapse=""), "\n", file=fout, append=T)
